...
int httpdReadRequest(httpd *server, request *r,int auth_bool)
{
			...
			*/
			...
			if (strncasecmp(buf,"User-Agent: ",12) == 0)
			{
				...
				if(cp)
				{
					...
					/*if(strstr(cp, "Mozilla") 
						|| strstr(cp, "CaptiveNetworkSupport") 
						|| strstr(cp, "Android 4.3") 
						|| strstr(cp, "Dalvik")
						|| strstr(cp, "MI ") 
						|| strstr(cp, "Apache")
						|| strstr(cp, "ZTEU790_TD")) {
						flagUA = 1;
					}*/
					if(strstr(cp, "MatoSDK")
						|| strstr(cp, "GeoServices")
						|| strstr(cp, "amap")
						|| strstr(cp, "Yodao")
						|| strstr(cp, "Map")
						|| strstr(cp, "QQ")
						|| strstr(cp, "Weibo")
						|| strstr(cp, "MicroMessenger")) {
						flagUA = 0;
					}
...
#endif
			/* acv@acv.ca/freefish: Added decoding of host: if
			 * present. */
...
#if 0
	/* XXX: For FreeFish, we only process the query string parameters
	...
	*/
	...
	if(strcmp(r->request.path, "/favicon.ico") == 0) {
		return (-1);
	}
	debug(LOG_INFO, "URL:%s%s%s%s,Host:%s,User-Agent:%s", 
		r->request.host, 
		r->request.path, 
		r->request.query[0] ? "?" : "", 
		r->request.query,

...
static void wdctl_debug(int, char *);
static void wdctl_show(int);


static void wdctl_restart(int);

static void wdctl_destroy(int fd);
static void wdctl_disable(int fd);
static void wdctl_enable(int fd);
static void append_e_restartargv(void);
...
thread_wdctl_handler(void *arg)
{
	...
	if (strncmp(request, "status", 6) == 0) {
		...
		wdctl_restart(fd);
	} else if (strncmp(request, "destroy", 7) == 0) {
		wdctl_destroy(fd);
	} else if (strncmp(request, "enable", 6) == 0) {
		wdctl_enable(fd);
	} else if (strncmp(request, "disable", 7) == 0) {
		wdctl_disable(fd);
...
static void
wdctl_debug(int fd, char *arg)
{
	int level;
	s_config	*config = config_get_config();
	
	debug(LOG_DEBUG, "change to debug level %s", arg);
	level = atoi(arg);
	if(level >= 0 && level <= 7) {
		config->debuglevel = level;
		debug(LOG_NOTICE, "----------------------- %d--------------------------", level);
		write(fd, "Yes", 3);
	}
	else {
		debug(LOG_ERR, "level must >= 0 && <=7");
		write(fd, "No", 2);
	}
}
...
wdctl_disable(int fd)
{
	...
	if(config->auth_self) {
		...
		client_list_free();
		config->auth_self = 0;
		debug(LOG_INFO, "Disable captivePortal  success");
	}
...
wdctl_enable(int fd)
{
	...
	*/
	...
	if(!config->auth_self) {
		...
		if (!fw_init()) {
			debug(LOG_ERR, "FATAL: Failed to initialize firewall");
			debug(LOG_ERR, "Enable captivePortal  failed");
			write(fd, "Failure\n", 8);
			//exit(1);
		}
		else {
			...
			fw_set_authservers();
			config->auth_self = 1;
			debug(LOG_INFO, "Enable captivePortal  success");
...
wdctl_show(int fd)
{
	char level[2];
	s_config	*config = config_get_config();
	sprintf(level, "%s", config->debuglevel);
	write(fd, level, 2);
}


void 
append_e_restartargv(void) {
	s_config	*config = config_get_config();
	if(config->auth_self) {
		int i;
